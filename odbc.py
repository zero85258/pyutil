        ...
        try:
            ...
            if not self._conn:
                self._conn = pyodbc.connect(
                    'DRIVER={SQL Server};Trusted_connection=no;' + 'SERVER=' + self._server + ';DATABASE=' + self._db + ';UID=' + self._user + ';PWD=' + self._user + ';'
                )
    ...
    def open_conn_with_retry(self, cnt: int) -> None:
        ...
        while ((not self._conn) or self._conn.closed != 0) and run_seq <= cnt:
            try:
                ...
                self._log.info(
                    "Connection to ODBC established: " + self._server + "/" + self._db + "\n"
                )
            except pyodbc.Error as e:
                self._log.info(
                    "Fail to connect to ODBC in " + str(run_seq) + "st run.\n"
                )
                self._log.error_stack(e)
                time.sleep(5)
            finally:
